...
Also note that because the contrast codes are applied to the variable in your data frame, any model you run with that variable will use those contrast codes. This will not be an issue in this course, but it's good to know about. The deep dive into setting contrasts in R in Learning More shows alternative ways of setting contrasts if you are ever in a situation where this behaviour would be an annoyance.
...
The formula to calculate the effect size for contrast results is (note this is equivalent to the complex formula in your lab slides):

$$
d = \frac{2 * \mathrm{estimate}}{\mathit{SD}_R * \sum_{k = 1}^k{| c_k |}}
$$
