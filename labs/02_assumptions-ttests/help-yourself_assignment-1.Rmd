...
```{r read-csv}
csv_data <- readr::read_csv(here::here("data", "help-yourself_a1.csv"))
csv_data
```
...
```{r read-spss}
sav_data <- haven::read_sav(here::here("data", "help-yourself_a1.sav"))
sav_data
```
...
csv_data_paired <- dplyr::mutate(csv_data, case = rep(1:12, times = 2))
csv_data_paired <- tidyr::pivot_wider(
  csv_data_paired, names_from = group, values_from = anxiety
)
csv_data_paired <- janitor::clean_names(csv_data_paired)

# First pivot the data to long format
csv_data_long <- tidyr::pivot_longer(
  csv_data_paired,
...
anxiety_scores <- dplyr::pull(csv_data_long, anxiety)
...
# function.
csv_data_long <- dplyr::group_by(csv_data_long, case) 
csv_data_long <- dplyr::mutate(
  csv_data_long,
...
ggpubr::ggline(csv_data_long, x = "condition", y = "anxiety_adj", add = "mean_ci")
```
...
```{r describeBy}
psych::describeBy(csv_data, group = "group", type = 2)
```
...
shapiro_test(csv_data, var = anxiety, group = group)
```
